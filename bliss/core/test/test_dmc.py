...
def test_leap_second_attrs():
    bliss.config.leapseconds._config['filename'] = os.path.join(
...
def test_update_leap_second_data():
    ...
    assert os.path.isfile(bliss.config.leapseconds.filename)
    os.remove(bliss.config.leapseconds.filename)
