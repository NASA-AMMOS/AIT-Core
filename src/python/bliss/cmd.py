...
class CmdDictCache(object):
    def __init__(self, filename=None):
        if filename is None:
            filename = bliss.Config.CMDDict.filename
    ...
    def load(self):
        if self.cmddict is None:
            if self.dirty():
                self.cmddict = CmdDict(self.filename)
                self.update()
            else:
                with open(self.pcklname, "rb") as stream:
                    self.cmddict = cPickle.load(stream)
    ...
    def update(self):
        ...
        with open(self.pcklname, "wb") as output:
            cPickle.dump(self.cmddict, output, -1)


_DefaultCmdDictCache = CmdDictCache()


def getDefaultCmdDict():
    cmddict = None

    try:
        filename = _DefaultCmdDictCache.filename
        cmddict  = _DefaultCmdDictCache.load()
    except IOError, e:
