...
class DictCache(object):
    def __init__(self, filename=None):
        if filename is None:
            filename = bliss.Config.TLMDict.filename
    ...
    def load(self):
        if self.gdsdict is None:
            if self.dirty():
                self.gdsdict = TlmDict(self.filename)
                self.update()
            else:
                with open(self.pcklname, "rb") as stream:
                    self.gdsdict = cPickle.load(stream)
    ...
    def update(self):
        ...
        with open(self.pcklname, "wb") as output:
            cPickle.dump(self.gdsdict, output, -1)
...
def getDefaultDict():
    ...
    try:
        ...
        dfltdict = _DefaultDictCache.load()
    except IOError, e:
        msg = "Could not load default telemetry dictionary '%s': %s'"
