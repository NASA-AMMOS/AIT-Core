...
class SocketStreamCapturer(object):
    ...
    def __init__(self, capture_handlers, address, conn_type):
        ...
        if conn_type == "udp":
            ...
            if RAW_SOCKET_FD:
                self.socket = gevent.socket.fromfd(
                    RAW_SOCKET_FD,
                    socket_family,
                ...
                self.socket = gevent.socket.socket(
                    socket_family, gevent.socket.SOCK_RAW, socket.htons(ETH_PROTOCOL)
                )
    ...
    def dump_handler_config_data(self):
        ...
        for h in self.capture_handlers:
            config_data.append(
                {
                    "handler": {k: v for k, v in h.items() if k not in ignored_keys},
    ...
    def dump_all_handler_stats(self):
        ...
        for h in self.capture_handlers:
            ...
            approx_data_rate = "{} bytes/second".format(
                h["data_read"] / float(time_delta)
            )
...
class StreamCaptureManagerServer(Bottle):
    ...
    def _route(self):
        ...
        self._app.route("/<name>/config", method="GET", callback=self._get_logger_conf)
        self._app.route(
            "/<name>/rotate", method="POST", callback=self._rotate_capturer_log
        )
