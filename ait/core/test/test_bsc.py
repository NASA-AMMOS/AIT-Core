...
class TestSocketStreamCapturer(object):
    ...
    def test_mocked_udp_socket(self, socket_mock):
        ...
        socket_mock.assert_called_with(gevent.socket.AF_INET, gevent.socket.SOCK_DGRAM)
    ...
    def test_mocked_eth_socket(self, socket_mock):
        ...
        if not bsc.RAW_SOCKET_FD:
            socket_mock.socket.assert_called_with(
                socket_family, gevent.socket.SOCK_RAW, socket.htons(proto)
            )
        else:
...
class TestStreamCaptureManager(object):
    ...
    @mock.patch("os.makedirs")
    @mock.patch("gevent.socket.socket")
    def test_bad_builtin_transform_load(
        self, socket_mock, mkdirs_mock, open_mock, log_mock
    ):
