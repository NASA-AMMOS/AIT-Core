...
def test_replaceVariables():
    ...
    # permutations
    pathvars = {"x": "x1", "y": ["y1", "y2"], "z": ["z1", "z2"]}
    pathname = os.path.join("/", "${x}", "${y}", "${z}", "ait-orbits")
    expected = [
        os.path.join(
            "/", pathvars["x"], pathvars["y"][0], pathvars["z"][0], "ait-orbits"
        ),
        os.path.join(
            "/", pathvars["x"], pathvars["y"][0], pathvars["z"][1], "ait-orbits"
        ),
        os.path.join(
            "/", pathvars["x"], pathvars["y"][1], pathvars["z"][0], "ait-orbits"
        ),
        os.path.join(
