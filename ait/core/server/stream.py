...
from ait.core import MAX_PORT
from ait.core import MIN_PORT
    ...
    if type(inputs) is int and MIN_PORT <= inputs <= MAX_PORT:
        ...
        if len(inputs) == 3 and type(inputs[0]) is str and inputs[0].upper() == "TCP":
            if type(inputs[1]) is str and inputs[1].lower() in [
                ...
                if type(inputs[2]) is int and MIN_PORT <= inputs[2] <= MAX_PORT:
